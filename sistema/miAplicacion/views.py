...
def cliente_lista(request):
    ...
    Returns:
        HttpResponse: Renderiza la plantilla 'cliente_lista.html' con el contexto de todos los clientes.
    """
...
def cliente_agregar(request):
    """
    Muestra un formulario para agregar un nuevo cliente y procesa los datos si se envían.

    Args:
        request (HttpRequest): La solicitud HTTP recibida.

    Returns:
        HttpResponse: Si el formulario es válido, redirige a 'cliente_lista'. 
                      De lo contrario, renderiza la plantilla 'cliente_form.html' con el formulario vacío o con errores.
    """
    if request.method == 'POST':
...
def cliente_editar(request, pk):
    """
    Muestra un formulario para editar un cliente existente y procesa los datos si se envían.

    Args:
        request (HttpRequest): La solicitud HTTP recibida.
        pk (int): El identificador primario del cliente a editar.

    Returns:
        HttpResponse: Si el formulario es válido, redirige a 'cliente_lista'. 
                      De lo contrario, renderiza la plantilla 'cliente_form.html' con el formulario y datos existentes o errores.
    """
...
def cliente_eliminar(request, pk):
    """
    Muestra una página de confirmación para eliminar un cliente y procesa la eliminación si se confirma.

    Args:
        request (HttpRequest): La solicitud HTTP recibida.
        pk (int): El identificador primario del cliente a eliminar.

    Returns:
        HttpResponse: Si se confirma la eliminación, redirige a 'cliente_lista'. 
                      De lo contrario, renderiza la plantilla 'cliente_confirm_delete.html' para la confirmación de eliminación.
    """
    cliente = get_object_or_404(Cliente, pk=pk)
    if request.method == 'POST':
        cliente.delete()
